...
class CollaborativeFiltering(object):
    ...
    def get_cosine_similarity(self, vector1, vector2):
        ...
        if isinstance(vector1, pd.Series) and isinstance(vector2, pd.Series):
            ...
            return (df * sr).sum(axis = 1) / (np.sqrt((df ** 2).sum(axis = 1)) * np.sqrt((sr ** 2).sum()))
    ...
    def get_k_similar(self, data, vector):
        ...
        # class
        normalize_data = self.normalize_data(data, self.get_row_mean(data))
        normalize_vector = self.normalize_data(vector, self.get_row_mean(vector)) 
        ...
        # in this class
        cos_sim = self.get_cosine_similarity(normalize_data, normalize_vector)
        ...
        k_sim = cos_sim.nlargest(n = self.k)   
    ...
    def get_rating(self, data, index, column):
        ...
        # ratings for the k similar items.
        top = (new_data.iloc[:,column][k_sim[0].values].values * k_sim[1].values).sum()
        bot = k_sim[1].values.sum()
